...
async def main():
    ...
    try:
        ...
        already_in_bootloader = await dfu.jump_to_bootloader(app_device)

        if already_in_bootloader:
            logger.info("Device is already in bootloader mode. Proceeding directly to update...")
            bootloader_device = app_device
        else:
